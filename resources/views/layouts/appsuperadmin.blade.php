<!DOCTYPE html>
<html lang="en" x-data="{ sidebarCollapsed: false }" class="h-full bg-gray-50">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FaceTend - Superadmin</title>
    ...
    <aside :class="sidebarCollapsed ? 'w-20' : 'w-64'"
        ...
        <div>
            ...
            <nav class="space-y-2 text-sm font-medium">
                <a href="{{ route('superadmin.dashboard') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded hover:bg-[#1E4F89] {{ request()->routeIs('superadmin.dashboard') ? 'bg-[#1E4F89]' : '' }}">
                    <i class="fas fa-home w-5 text-center text-lg"></i>
                    <span x-show="!sidebarCollapsed">Dashboard</span>
                </a>

                <a href="{{ route('superadmin.admin.admin-management.index') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded hover:bg-[#1E4F89] {{ request()->routeIs('superadmin.admin.admin-management.*') ? 'bg-[#1E4F89]' : '' }}">
                    <i class="fas fa-users-cog w-5 text-center text-lg"></i>
                    <span x-show="!sidebarCollapsed">Admin Management</span>
                </a>

                <a href="{{ route('superadmin.logs.index') }}"
                   class="flex items-center gap-3 px-3 py-2 rounded hover:bg-[#1E4F89] {{ request()->routeIs('superadmin.logs.index') ? 'bg-[#1E4F89]' : '' }}">
                    <i class="fas fa-file-alt w-5 text-center text-lg"></i>
                    <span x-show="!sidebarCollapsed">Logs</span>
                </a>
            </nav>
        </div>
        ...
        <div x-data="{ showDropdown: false }" class="relative" x-show="!sidebarCollapsed" x-transition.opacity>
            <div @click="showDropdown = !showDropdown"
                 class="cursor-pointer flex items-center justify-between px-3 py-2 rounded hover:bg-[#1E4F89]">
            ...
            <div x-show="showDropdown" @click.outside="showDropdown = false"
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                            class="w-full px-4 py-2 text-sm hover:bg-gray-100 text-left flex items-center gap-2">
